...
def run(ed_path=ED_PATH, tri_path=TRI_PATH, adm_path=ADM_PATH):
    ...
    # 4) Use raw acuity as category
    df["acuity_level"] = df["acuity"].astype(int)

    # 5) Console summaries
    print("\n=== Acuity-level probabilities ===")
    acuity_probs = df["acuity_level"].value_counts(normalize=True).sort_index()
    print(acuity_probs.to_string())

    print("\n=== LOS stats by raw acuity (1-5) ===")
    los_raw = pd.DataFrame({lvl: summary(g["hospital_hours"]) 
                              for lvl, g in df.groupby("acuity_level")}).T
    print(los_raw.to_string())

    trimmed = df[df["hospital_hours"] < MAX_LOS_H]
    print("\n=== LOS stats by acuity level (<30 days) ===")
    los_by_level = pd.DataFrame({lvl: summary(g["hospital_hours"]) 
                                   for lvl, g in trimmed.groupby("acuity_level")}).T
    print(los_by_level.to_string())
    ...
    for lvl, sub in trimmed.groupby("acuity_level"):
        ...
        fitted[lvl] = (mu, sigma)
        print(f"Acuity {lvl}: mu={mu:.3f}, sigma={sigma:.3f}, n={len(sub)}")

    kw = kruskal(*[g["hospital_hours"] for _, g in trimmed.groupby("acuity_level")])
    ...
    # 7) Plot figures for acuity_level
    # 7.1 Frequency bar
    fig1, ax1 = plt.subplots(figsize=(6,4))
    acuity_probs.plot(kind="bar", ax=ax1)
    ax1.set_ylabel("Proportion of arrivals")
    ax1.set_xlabel("Acuity Level")
    ax1.set_title("Frequency by Acuity Level")
    fig1.tight_layout()
    savefig(fig1, "fig01_acuity_frequency")
    plt.show()

    # 7.2 Violin + box LOS
    fig2, ax2 = plt.subplots(figsize=(8,5))
    sns.violinplot(data=trimmed, x="acuity_level", y="hospital_hours",
                   inner="box", ax=ax2)
    ax2.set_ylim(0, MAX_LOS_H)
    ax2.set_ylabel("Hospital LOS (h)")
    ax2.set_xlabel("Acuity Level")
    ax2.set_title("LOS by Acuity Level (<30 days)")
    fig2.tight_layout()
    savefig(fig2, "fig02_los_violin")
    plt.show()

    # 7.3 Density histogram per level
    fig3, ax3 = plt.subplots(figsize=(8,5))
    sns.histplot(data=trimmed, x="hospital_hours", hue="acuity_level",
                 element="step", stat="density", common_norm=False,
                 binwidth=6, ax=ax3)
    ax3.set_xlim(0, MAX_LOS_H)
    ax3.set_xlabel("Hospital LOS (h)")
    ax3.set_title("Density of LOS by Acuity Level (<30 days)")
    fig3.tight_layout()
    ...
    for lvl, sub in trimmed.groupby("acuity_level"):
        ...
        ax4.step(x, y, label=str(lvl))
    ax4.set_xlim(0, MAX_LOS_H)
    ax4.set_xlabel("Hospital LOS (h)")
    ax4.set_ylabel("ECDF")
    ax4.set_title("ECDF of LOS by Acuity Level (<30 days)")
    ax4.legend(title="Acuity")
    fig4.tight_layout()
    ...
    ed_sorted["hour"].value_counts().sort_index().plot(kind="bar", ax=ax7)
    ...
    return lambda_hour, acuity_probs, fitted

# ------------------------------ CLI entry ------------------------------ #
if __name__ == "__main__":
    lam24, acuity_probs, fitted = run()
    artefacts = {
        "lambda_hour":   lam24.reindex(range(24)).fillna(0).tolist(),
        "acuity_probs":  acuity_probs.sort_index().tolist(),
        "lognorm_params": {lvl: [float(mu), float(sigma)] for lvl, (mu, sigma) in fitted.items()}
    }
